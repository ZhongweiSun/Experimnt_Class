...
class LY12_txt:
    ...
    def test_modu_MS(self):
        ...
        from scipy.stats import linregress
#        for i in range(0, self._n-51, 30) :
#            slope, intercept, r_value, _, _ = linregress(abs(self._dis)[i:i+50], abs(self._load)[i:i+50])
#             slope, self.inter_MS, r_value, _, _ = linregress(self._dis[i:i+50], self._load[i:i+50])
        for i in range(0, self._n-1001, 50):
            slope, intercept , r_value, _, _ = linregress(abs(self._dis)[i:i+1000], abs(self._load)[i:i+1000])
#        for i in range(0, self._n - 401, 50):
