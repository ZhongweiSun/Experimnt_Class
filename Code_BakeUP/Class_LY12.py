...
class LY12_txt:
    ...
    num_test_data = 0                                             # The number of test data files

    global LY12_dict, test_date
    # TODO The attributes of test pieces (LY12)
    def __init__(self, file_name):
        ''' Get attributes of test pieces data '''

        name_info = file_name.split('.')[0].split('_')           # file_name: LY12_6_A1_20190507_11.txt
        self.piece_num = '_'.join(name_info[:3])                 # piece name + test number, LY12_6_A1
        self.name = file_name.split('.')[0]                      # LY12
        self.num_exp = name_info[1]                              # Several experiments, 6
        ...
        self.description = name_info[4]                          # The description

        # The size of test piece
        ...
        self._load = self._raw_data[:, 0]
        self._dis = self._raw_data[:, 1]                                              # The displacement of Extensometer(引伸计)
        self._Dis = self._raw_data[:, 2]                                              # The displacement of Testing Machine
        self.ult_load = self.Ult_load()
        self.ult_stre = self.ult_load / self.area
        self._n = len(self._load)

        self.error_ults = (self.ult_stre - self.LY12_ultS) / self.LY12_ultS * 100                   # The deviation between ult_stre and LY12_ultS
        self.error_ult_GB = (self.ult_stre - self.LY12_ultS_min_GB) / self.LY12_ultS_min_GB * 100   # The deviation between ult_stre and LY12_ultS_min_GB
    ...
    def test_modu_0307(self):
        ...
        if self.r_value_0307 < 0.9:
            print(self.name, '0307 Mthod: Correlation coefficient < 0.9')
        self.data_plot = np.row_stack(([[0,0]],np.column_stack((self._dis[self.i_min_0307:] + self.inter_0307/self.stiff_0307, \
            self._load[self.i_min_0307:]))))

        self.test_yieldL_0307 = LY12_txt.cal_yield(self.data_plot[:, 1], self.data_plot[:, 0], self.i_min_0307, self.height, self.stiff_0307)
        self.test_yieldS_0307 = self.test_yieldL_0307 / self.area
        self.test_yieldR_0307 = self.test_yieldL_0307 / self.ult_load             # The yield ratio of test piece
        return self.stiff_0307 / self.area * self.height

    @property
    def test_modu_MS(self):
        ...
        from scipy.stats import linregress
        # for i in range(self._n - 51) :
        #     slope, self.inter_MS, r_value, _, _ = linregress(abs(self._dis)[i:i+50], abs(self._load)[i:i+50])
            # slope, self.inter_MS, r_value, _, _ = linregress(self._dis[i:i+50], self._load[i:i+50])
        for i in range(0, self._n - 101, 20):
        ...
        if Rmax < 0.9:
            print(self.name, 'Maxmium slope method: Correlation coefficient < 0.9')
            print(self.name, 'Maxmium slope method, intercept = ', Imax)
        self.data_plot = np.row_stack(([[0,0]], np.column_stack((self._dis[self.i_max_maxS:] \
            + Imax/Smax, self._load[self.i_max_maxS:]))))

        self.stiff_MS = Smax
        self.inter_MS = Imax
        self.r_value_maxSlope = r_value

        # self.test_yieldL_MS = LY12_txt.cal_yield(self._load, self._dis, self.i_max_maxS, self.height, \
        #     self.stiff_MS, self.inter_MS)
        self.test_yieldL_MS = LY12_txt.cal_yield(self.data_plot[:, 1], self.data_plot[:, 0], self.i_max_maxS, self.height, self.stiff_MS)
        self.test_yieldS_MS = self.test_yieldL_MS / self.area
        self.test_yieldR_MS = self.test_yieldL_MS / self.ult_load

        # import matplotlib.pyplot as plt
        # plt.figure()
        # plt.plot(self._dis, self._load, 'b')
        # # plt.plot(self.data_plot[:, 0], self.data_plot[:, 1])
        # plt.plot(self._dis + 0.002*self.height, self.inter_MS + self.stiff_MS * self._dis, 'k')
        # plt.show()

        return self.stiff_MS/self.area * self.height
    ...
    def test_modu_SA(self):
        ...
        #data_plot数据计算有问题
        self.data_plot = np.row_stack(([[0,0]],np.column_stack((self._dis[load_01:] + intercept/stiff, self._load[load_01:]))))

        self.test_yieldL_SA = yield_load
        self.test_yieldS_SA = self.test_yieldL_SA/self.area
        self.test_yieldR_SA = self.test_yieldL_SA/self.ult_load
        self.stiff_SA = stiff
...
class LY12_pieces:
    ...
    def Yield_0307(self):
        ...
        self.yield_0307 = LY12_txt.cal_yield(self.second.data_plot[:, 1], self.second.data_plot[:, 0], \
            self.second.i_max_0307, self.second.height, self.second.stiff_0307)    
        return self.yield_0307/self.second.area  

    @property
    def Yield_MS(self):
        
        self.yieldL_MS = LY12_txt.cal_yield(self.second.data_plot[:, 1], self.second.data_plot[:, 0], 300, \
    ...
    def Plt_Dis_Load(self):
        ...
        import matplotlib.pyplot as plt
        plt.figure()
        plt.plot(self.first._dis, self.first._load, label = self.first.piece_num)
        plt.plot(self.second._dis, self.second._load, label = self.second.piece_num)
        plt.plot(self.third._dis, self.third._load, label = self.third.piece_num)
        plt.legend()
